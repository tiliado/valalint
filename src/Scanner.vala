 ...
 */
public class Vala.Scanner2 {
    public SourceFile source_file { get; private set; }
    public bool use_preprocessor { get; private set; }
    ...
    public Scanner2 (SourceFile source_file, bool use_preprocessor=true) {
    ...
    void parse_pp_if () {
        ...
        bool condition = parse_pp_expression() || !use_preprocessor;
    ...
    void parse_pp_else () {
        ...
        if (!use_preprocessor || !conditional_stack[conditional_stack.length - 1].matched
        && (conditional_stack.length == 1 || !conditional_stack[conditional_stack.length - 2].skip_section)) {
            // condition true => process code within if
            conditional_stack[conditional_stack.length - 1].matched = true;
            conditional_stack[conditional_stack.length - 1].skip_section = false;
    ...
    bool whitespace () {
        ...
        if (bol && current < end && current[0] == '#' && use_preprocessor) {
            pp_directive();
            return true;
        }
        return found;
